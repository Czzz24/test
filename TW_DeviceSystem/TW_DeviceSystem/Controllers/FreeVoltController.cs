...
{
    [AccountAuthorize]
    public class FreeVoltController : Controller
    {
        private readonly static string conStr = ConfigurationManager.ConnectionStrings["constrMain"].ToString();
        private readonly static t_freeVoltServicingBLL bll = new t_freeVoltServicingBLL();
        ...
        public virtual JsonResult GetHistoryData(int page, int limit, string TerminalId, int ElectricId, int LineId, DateTime? startTime, DateTime? endTime)
        {
            ...
            List<t_freeVoltServicing> list = bll.GetHistoryData(conFstr, TerminalId, page, limit, startTime, endTime, out totalcount);
            uniteModel<t_freeVoltServicing> model = new uniteModel<t_freeVoltServicing>
            {
                code = 0,
                msg = "免维护环压历史数据列表查询成功!",
                count = totalcount,
        ...
        public virtual JsonResult GetSingle(string TerminalId, int ElectricId, int LineId)
        {
            FromConnection fromCon = new FromConnection();
            string conFstr = fromCon.GetConStr(conStr, ElectricId, LineId);
            t_freeVoltServicing model = bll.GetSingle(conFstr, TerminalId);
            return Json(model, JsonRequestBehavior.AllowGet);
        }
        ...
        public virtual JsonResult GetChartData(string TerminalId, int ElectricId, int LineId, DateTime? startTime, DateTime? endTime)
        {
            ...
            List<FreeVoltServiceChart> list = bll.GetChartData(conFstr, TerminalId, startTime, endTime);
            ...
            for (int h = 0; h < list.Count; h++)
            {
                ...
                AElectric.Add(list[h].AVolt);
                BElectric.Add(list[h].BVolt);
                CElectric.Add(list[h].CVolt);
                TElectric.Add(list[h].TVolt);
